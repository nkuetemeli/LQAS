...
def train(params, filename, classes, train_loader, val_loader, test_loader):
    # Architecture search
...
if __name__ == '__main__':
    ...
    # model.candidate_architectures = model.candidate_architectures[:20]
    ...
    ft = 'ftFalse'
    fine_tuning = True if ft == 'ftTrue' else False
    evolution = EvolutionarySearch(super_model=model)
    evolution.evolve(train_loader, val_loader, test_loader, fine_tuning,
                     filename=f'ModelsFL'+str(params.frozen_linear)+'/evolutionary_' + params.which_model_net + '_evolved_'+ft+'.sav')


    for i, acc in enumerate(evolution.top_k_accuracies):
        plt.scatter([i]*len(acc), acc, c='g')
    plt.show()